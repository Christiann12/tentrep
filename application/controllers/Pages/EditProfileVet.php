...
class EditProfileVet extends CI_Controller {
        ...
        if($this->form_validation->run() === true){
            if(!empty($_FILES['attachment']['name'])){
                ...
                //if upload success update the database with the correct filename
                    ...
                    if($this->Vet_model->editUser($postData,$this->session->userdata('userId'))){
    ...
    public function saveEditClinicInformation(){
        $this->form_validation->set_rules('clinicName', 'Clinic Name' ,'required');
        ...
        //load config for upload library
		$config['upload_path']   = APPPATH.'assets/images/clinicpicture/';
		$config['allowed_types'] = 'jpg|jpeg|jpe|png';
		$config['max_size']      = 0;
		$config['max_width']     = 0;
		$config['max_height']    = 0;
        ...
        if($this->form_validation->run() === true){
            if(!empty($_FILES['clinicpicture']['name'])){
                //if upload fails abort process and display error
                if ( ! $this->upload->do_upload($name) ) {
                    $this->session->set_flashdata('failEditVet',$this->upload->display_errors());
                } 
                //if upload success update the database with the correct filename
                    ...
                    $postData['clinicpicture'] = $upload['file_name'];

                    if($this->Vet_model->editUser($postData,$this->session->userdata('userId'))){
                        $this->session->set_flashdata('successEditVet','Edit Success');
                        if($this->session->userdata('clinicpicture') != 'empty'){
                            unlink(APPPATH.'assets/images/clinicpicture/'.$this->session->userdata('clinicpicture'));
                        }
                        $this->session->set_userdata([
                            'isLogIn'     => true,
                            'clinicpicture'       => $upload['file_name'],
                        ]);
                    }
                    else{
                        $this->session->set_flashdata('failEditVet','Edit Failed');
