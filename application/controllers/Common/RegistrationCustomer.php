...
class RegistrationCustomer extends CI_Controller {
	...
	{
		$this->load->view('Common/RegistrationCustomer');
	}
	public function saveUser(){
		...
		$postData = array(
            "customerId" => "CSTM-".$this->randStrGen(2,7),
		...
		if($this->form_validation->run() === true){
			if ($this->Customer_model->addUser($postData)){
				$this->session->set_flashdata('successCustomerCreate','Add Success');
			}
			else{
				$this->session->set_flashdata('errorCustomerCreate','Add Failed');
			}
            redirect('RegistrationCustomer');
        }
        else{
			$this->session->set_flashdata('errorCustomerCreate',validation_errors());
            redirect('RegistrationCustomer');
        }
	}
	// ---------------------------------------------CUSTOM FUNCTIONS---------------------------------------------
