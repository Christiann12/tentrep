...
class RegistrationVet extends CI_Controller {
	...
	{
		$this->load->view('Common/RegistrationVet');
	}
	public function saveUser(){
		...
		if($this->form_validation->run() === true){
			if ($this->Vet_model->addUser($postData)){
				$this->session->set_flashdata('successVetCreate','Add Success');
			}
			else{
				$this->session->set_flashdata('errorVetCreate','Add Failed');
			}
            redirect('RegistrationVet');
        }
        else{
			$this->session->set_flashdata('errorVetCreate',validation_errors());
            redirect('RegistrationVet');
        }
	}
